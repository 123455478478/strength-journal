...
(async () => {
  ...
  await page.getByRole("button", { name: /记录附加重量/ }).click();
  if (!await page.getByLabel("附加重量（可选）").isVisible()) throw new Error("自重动作缺少可选附重入口");
  await page.getByLabel("附加重量（可选）").fill("10");
  ...
  if (!await page.getByText("附重 10 kg · 30 秒", { exact: true }).isVisible()) throw new Error("自重附重表现显示错误");
  await page.getByRole("button", { name: "完成该动作" }).click();
  await page.getByRole("button", { name: "结束健身" }).click();
  if (!(await page.locator(".set-summary").textContent()).includes("附重 10 kg · 30 秒")) throw new Error("训练总结未保留自重附重记录");
  await page.evaluate(() => localStorage.clear());
  await page.reload();
